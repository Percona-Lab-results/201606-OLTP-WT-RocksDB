...
title <- "[network] sysbench OLTP READ-ONLY in memory / samsung SM863"
p2 = dbGetQuery( con,'SELECT sec,sr.runid runidfull, sr.threads, cachesize, engine, workload, tps, rt 
                 FROM sb_mongo_results sr 
...
title <- "[network] sysbench OLTP READ-WRITE iobound / samsung SM863"
p2 = dbGetQuery( con,'SELECT sec,sr.runid runidfull, sr.threads, cachesize, engine, workload, tps, rt 
                 FROM sb_mongo_results sr 
...
m+geom_boxplot()+geom_point( position = position_jitterdodge())+expand_limits(y=0)+
  labs(x="Threads",y="Throughput, tps",title=title)+tm+scale_color_tableau(name="Engine")
...
title <- "[network] sysbench OLTP READ-WRITE iobound / samsung SM863 / 112 threads"
p2 = dbGetQuery( con,'SELECT sec,sr.runid runidfull, sr.threads, cachesize, engine, workload, tps, rt 
                 FROM sb_mongo_results sr 
...
m+geom_point()+geom_line()+expand_limits(y=0)+
  labs(x="--- time, sec [10 sec intervals] --->",y="Throughput, tps",title=title)+tm+scale_color_tableau(name="Engine")
...
title <- "[network] sysbench OLTP READ-WRITE io-bound / samsung 850 Pro"
p2 = dbGetQuery( con,'SELECT sec,sr.runid runidfull, sr.threads, cachesize, engine, workload, tps, rt 
                 FROM sb_mongo_results sr 
...
title <- "[network] sysbench OLTP READ-ONLY in memory / samsung PM1725"
p2 = dbGetQuery( con,'SELECT sec,sr.runid runidfull, sr.threads, cachesize, engine, workload, tps, rt 
                 FROM sb_mongo_results sr 
...
title <- "[network] sysbench OLTP READ-WRITE io-bound / samsung PM1725"
p2 = dbGetQuery( con,'SELECT sec,sr.runid runidfull, sr.threads, cachesize, engine, workload, tps, rt 
                 FROM sb_mongo_results sr 
...
title <- "[network] sysbench OLTP READ-WRITE iobound"
p2 = dbGetQuery( con,'SELECT sec,sr.runid runidfull, sr.threads, cachesize, engine, workload, storage,tps, rt 
                 FROM sb_mongo_results sr 
